  ...
  if test "${TARGET+1}"; then
    case "${TARGET}" in
      *-linux-android*)
        ...
        ANDROID_ARCH="$(echo "${TARGET}" | cut -d- -f1)"
...
setup_source() {
  ...
  case "${PKG_SRCURL}" in
    ...
    *.tar.bz2) PKG_EXTNAME=.tar.bz2 ;;
    git+*)
      url=$(echo $PKG_SRCURL | cut -d+ -f2-)
      msg "Cloning ${url} ..."

      if ! test -d "${BUILD_PREFIX}/${PKG}"; then
        git clone $url --branch $PKG_VERSION --depth 1 $BUILD_PREFIX/$package
      fi
      cd $BUILD_PREFIX/$package || exit
      git reset --hard
      cd "${BUILD_PREFIX}/${PKG}" || exit
...
setup_depends() {
  if test "${PKG_DEPENDS+1}"; then
    msg "Package '${PKG}' depends on '${PKG_DEPENDS}'"
    for dep in ${PKG_DEPENDS}; do
      (
        unset -f check
        # shellcheck disable=SC1090
        . "${ROOT_DIR}/packages/${dep}/build.sh"
...
build_package() {
  (
    PKG="$1"
    PKG_CONFIG_DIR="${ROOT_DIR}/packages/${PKG}"
    export PKG_CONFIG_DIR
    unset BUILD_PREFIX PKG_DEPENDS
    # shellcheck disable=SC1090
    . "${ROOT_DIR}/packages/${PKG}/build.sh"
    msg "Building package ${PKG} ${PKG_VERSION}"
    for step in setup_target setup_depends setup_source pre_configure configure post_configure build; do
