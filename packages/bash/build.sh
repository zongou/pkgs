...
PKG_DESCRIPTION="A sh-compatible shell that incorporates useful features from the Korn shell (ksh) and C shell (csh)"
PKG_LICENSE="GPL-3.0"
PKG_MAINTAINER="@termux"
_MAIN_VERSION=5.2
_PATCH_VERSION=26
PKG_VERSION=${_MAIN_VERSION}.${_PATCH_VERSION}
PKG_REVISION=1
PKG_SRCURL=https://mirrors.kernel.org/gnu/bash/bash-${_MAIN_VERSION}.tar.gz
PKG_SHA256=a139c166df7ff4471c5e0733051642ee5556c1cc8a4a78f145583c5c81ab32fb
PKG_AUTO_UPDATE=false
# PKG_DEPENDS="libandroid-support, libiconv, readline (>= 8.0), termux-tools"
PKG_DEPENDS="readline"
PKG_RECOMMENDS="command-not-found, bash-completion"
PKG_BREAKS="bash-dev"
PKG_REPLACES="bash-dev"
PKG_ESSENTIAL=true
PKG_BUILD_IN_SRC=true
PKG_BASENAME=bash-${_MAIN_VERSION}

# PKG_EXTRA_CONFIGURE_ARGS="--enable-multibyte --without-bash-malloc --with-installed-readline"
# PKG_EXTRA_CONFIGURE_ARGS+=" bash_cv_job_control_missing=present"
# PKG_EXTRA_CONFIGURE_ARGS+=" bash_cv_sys_siglist=yes"
# PKG_EXTRA_CONFIGURE_ARGS+=" bash_cv_func_sigsetjmp=present"
# PKG_EXTRA_CONFIGURE_ARGS+=" bash_cv_unusable_rtsigs=no"
# PKG_EXTRA_CONFIGURE_ARGS+=" ac_cv_func_mbsnrtowcs=no"
...
# PKG_EXTRA_CONFIGURE_ARGS+=" bash_cv_dev_fd=whacky"
...
# PKG_EXTRA_CONFIGURE_ARGS+=" bash_cv_getcwd_malloc=yes"

# PKG_CONFFILES="etc/bash.bashrc etc/profile"

# PKG_RM_AFTER_INSTALL="share/man/man1/bashbug.1 bin/bashbug"
...
# 		PATCHFILE=$PKG_CACHEDIR/bash_patch_${PATCH_NUM}.patch
# 		download \
...
# 		-e "s|@HOME@|$ANDROID_HOME|g" \
# 		$PKG_BUILDER_DIR/etc-profile > $PREFIX/etc/profile

# 	# /etc/bash.bashrc - System-wide .bashrc file for interactive shells. (config-top.h in bash source, patched to enable):
# 	sed -e "s|@PREFIX@|$PREFIX|g" \
# 		-e "s|@HOME@|$ANDROID_HOME|g" \
# 		$PKG_BUILDER_DIR/etc-bash.bashrc > $PREFIX/etc/bash.bashrc
# }
...
configure() {
	...
	export CFLAGS="-w"
	./configure --host="${TARGET}" --disable-nls --enable-static-link --without-bash-malloc bash_cv_dev_fd=whacky bash_cv_getcwd_malloc=yes --enable-multibyte --prefix="${OUTPUT_DIR}"
	make -j"${JOBS}" install
}
